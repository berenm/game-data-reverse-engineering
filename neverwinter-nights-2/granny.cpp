...
namespace gr2 {
  ...
  namespace {
    ...
    // #include "PLC_DC_TempleE01.gr2.hpp"
#include "N_KoS_UnA_cutsceesdeath01.gr2.hpp"
    ...
    struct weighted_window {
      ...
      weighted_window(uint32_t max_value, uint16_t count_cap);
    ...
    struct dictionary {
      ...
      weighted_window                lowbit_window;
      weighted_window                highbit_window;
      std::vector< weighted_window > midbit_windows;

      std::vector< weighted_window > decoded_windows;
      std::vector< weighted_window > size_windows;
    ...
    weighted_window::weighted_window(uint32_t max_value, uint16_t count_cap) {
      this->weight_total = 4;
    ...
    void weighted_window::rebuild_ranges() {
    ...
    void weighted_window::rebuild_weights() {
    ...
    auto weighted_window::try_decode(decoder & dec) {
    ...
    uint32_t dictionary::decompress_block(decoder& dec, uint8_t* dbuf) {
      ...
      if (this->backref_size > 0) {
        ...
        auto backref_offset = ((d4.second << 10) + (d5.second << 2) + d3.second + 1);

        this->decoded_size += backref_size;
        std::memmove(dbuf, dbuf - backref_offset, backref_size);
  ...
  void decompress(uint32_t csize, uint8_t* cbuf, uint32_t a4, uint32_t a5, uint32_t dsize, uint8_t* dbuf) {
